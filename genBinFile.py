...
def writeBinaray(outputFile, imagePath, label): # 바이너리 파일에 이미지 경로와 라벨값을 붙인다.
    ...
    img = img.resize((imageSize, imageSize), PIL.Image.ANTIALIAS)#  ANTIALIAS는 부드럽지만 조금은 연하지만, 포토샵의 손으로 문지른 효과처럼 약간은 번짐이 보이는 이미지와 같은 효과를 준다.
    img = (np.array(img))
...
subDirs = os.listdir(input) #input폴더 내의 sub directory 목록이 저장된다. subDirs는 배열.
...
for subDir in subDirs:#현재 디렉토리에 있는 하위 디렉토리를 하나씩 꺼내서
    ...
    # 라벨은 순차적으로 0, 1, 2, ...로 붙여지는 숫자다. subDir은 하위 디렉토리 이름이다.
